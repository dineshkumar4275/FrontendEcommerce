...
function HomeContent() {
  // ✅ Only import useApp inside this component
  const { useApp } = require('../src/hooks/useApp');
  const app = useApp();
...
export default function HomeClient() {
  ...
  useEffect(() => {
    setMounted(true);
    // ✅ Dynamically import useApp on client side only
    import('../src/hooks/useApp').then((module) => {
      setApp(module.useApp());
    });
  }, []);

  // ✅ Use fallback translations during SSR
  const t = (key) => {
    if (mounted && app) {
      return app.t(key);
    }
  ...
  const currency = mounted && app ? app.currency : { symbol: '₹', code: 'INR' };
  const currentLanguage = mounted && app ? app.currentLanguage : { name: 'Loading...', flag: '🌍' };
  ...
  const features = [ 
    { icon: TruckIcon, title: t('free_shipping'), desc: t('free_shipping_desc') },
    { icon: ShieldCheckIcon, title: t('secure_payment'), desc: t('secure_payment_desc') },
    { icon: ClockIcon, title: t('support'), desc: t('support_desc') },
    { icon: ArrowRightIcon, title: t('easy_returns'), desc: t('easy_returns_desc') }
  ];
  ...
  return (
    <>
      <SEO
        ...
        organization={organizationData}
        website={websiteData}
        breadcrumbs={[{ name: t('home'), url: 'https://www.sombu.in/' }]}
      />
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-black pt-20">
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-purple-950 to-black">
          <div className="relative container mx-auto px-4 py-16 md:py-24 text-center">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/60 backdrop-blur-md border border-slate-700 mb-6 animate-fade-down">
              <SparklesIcon className="w-4 h-4 text-yellow-400" />
              <span className="text-sm text-slate-300">{t('coming_soon')}</span>
            </div>

            {/* Title */}
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold mb-6 animate-fade-up">
              <span className="text-slate-200">{t('discover_your')}</span>
              <span className="block bg-gradient-to-r from-yellow-400 via-pink-400 to-purple-400 bg-clip-text text-transparent mt-2">
                {t('perfect_style')}
              </span>
            </h1>
            ...
            <div className="flex flex-wrap justify-center gap-4 animate-fade-up animation-delay-400">
              <div className="group inline-flex items-center gap-2 px-8 py-3 bg-slate-700/50 text-slate-400 rounded-full font-semibold cursor-not-allowed opacity-60">
                ...
                <ArrowRightIcon className="w-4 h-4" />
                <span className="text-xs bg-slate-600/50 px-2 py-0.5 rounded-full">{t('soon')}</span>
              </div>

              <div className="inline-flex items-center gap-2 px-8 py-3 bg-slate-800/60 backdrop-blur-md border border-slate-700 text-slate-400 rounded-full font-semibold cursor-not-allowed opacity-60">
                {t('view_collections')}
                <span className="text-xs bg-slate-700/50 px-2 py-0.5 rounded-full">{t('soon')}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-3xl mx-auto mt-16 pt-8 border-t border-slate-800 animate-fade-up animation-delay-600">
              <div>
                <div className="text-2xl font-bold text-slate-200">{t('soon')}</div>
                <div className="text-sm text-slate-500">{t('happy_customers')}</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-slate-200">{t('soon')}</div>
                <div className="text-sm text-slate-500">{t('brands')}</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-slate-200">24/7</div>
        ...
        <div className="container mx-auto px-4 py-16 bg-slate-950">
          <div className="max-w-2xl mx-auto text-center">
            <div className="text-7xl mb-6">🛍️</div>
            <h2 className="text-3xl font-bold text-slate-200 mb-4">{t('no_products')}</h2>
            <p className="text-slate-400 mb-2">{t('curating_collection')}</p>
            <p className="text-slate-500 text-sm mb-8">{t('under_construction')}</p>

            <div className="inline-flex items-center gap-4 px-6 py-4 bg-slate-800/60 rounded-2xl border border-slate-700">
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 bg-green-400 rounded-full animate-pulse"></div>
                <span className="text-slate-300">{t('launching_soon')}</span>
              </div>
              <div className="w-px h-6 bg-slate-700"></div>
              <div className="flex items-center gap-2">
                <span className="text-slate-500 text-sm">📅</span>
                <span className="text-yellow-400 text-sm font-medium">{t('stay_tuned')}</span>
              </div>
            </div>
            ...
            <div className="mt-8 max-w-md mx-auto">
              ...
              <p className="text-xs text-slate-600 mt-2">{t('almost_there')}</p>
            </div>
            ...
            {mounted && (
              <div className="mt-12 p-4 bg-slate-800/40 rounded-xl border border-slate-700">
                <p className="text-sm text-slate-400">
                  {t('current_language')}: <span className="text-slate-200 font-semibold">{currentLanguage?.flag} {currentLanguage?.name}</span>
                </p>
                <p className="text-sm text-slate-400 mt-1">
                  {t('current_currency')}: <span className="text-slate-200 font-semibold">{currency?.symbol} {currency?.code}</span>
                </p>
              </div>
            )}
          </div>
