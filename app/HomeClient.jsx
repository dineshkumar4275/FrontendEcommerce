...
export default function HomeClient() {
  // ✅ Get everything from useApp
  const { t, formatPrice, currency, currentLanguage, language, forceUpdate, changeLanguage } = useApp();
  
  const [mounted, setMounted] = useState(false);
  // ✅ Use a counter to force re-render
  ...
  // ✅ Translation function that updates with language
  const translate = useCallback((key) => {
    // ✅ Use the language from context directly
    if (mounted) {
      return t(key);
    }
    return FALLBACK_TRANSLATIONS[key] || key;
  }, [mounted, t, language, renderCount]); // ✅ Added renderCount to dependencies
  ...
  const formatPriceWithFallback = useCallback((amount) => {
    ...
    return `₹ ${Number(amount).toFixed(2)}`;
  }, [mounted, formatPrice, currency, renderCount]); // ✅ Added currency and renderCount

  // ✅ Memoize features to update when language changes
  const features = useMemo(() => [ 
    { icon: TruckIcon, title: translate('free_shipping'), desc: translate('free_shipping_desc') },
    { icon: ShieldCheckIcon, title: translate('secure_payment'), desc: translate('secure_payment_desc') },
    { icon: ClockIcon, title: translate('support'), desc: translate('support_desc') },
    { icon: ArrowRightIcon, title: translate('easy_returns'), desc: translate('easy_returns_desc') }
  ], [translate]); // ✅ Recompute when translate changes

  // ✅ Current language and currency with fallback
  const currentLang = useMemo(() => 
    mounted ? currentLanguage : { name: 'English', flag: '🇬🇧' },
    [mounted, currentLanguage, renderCount] // ✅ Added renderCount
  );

  const currentCurrency = useMemo(() => 
    mounted ? currency : { symbol: '₹', code: 'INR' },
    [mounted, currency, renderCount] // ✅ Added renderCount
  );
  ...
  // ✅ Test function to change language
  const testChangeLanguage = () => {
    const languages = ['en', 'ta', 'hi', 'te', 'ml'];
    const currentIndex = languages.indexOf(language);
    const nextIndex = (currentIndex + 1) % languages.length;
    const nextLang = languages[nextIndex];
    console.log('🔄 Testing language change to:', nextLang);
    changeLanguage(nextLang);
  };

  // ✅ Force a complete re-render when language changes
  const pageKey = `home-${language}-${renderCount}-${forceUpdate}-${Date.now()}`;
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-black pt-20">
        ...
        <div className="container mx-auto px-4 py-16 bg-slate-950">
          <div className="max-w-2xl mx-auto text-center">
            ...
            {/* ✅ Language & Currency Display */}
            <div className="mt-12 p-6 bg-slate-800/40 rounded-xl border border-slate-700">
              ...
              {/* ✅ Test button to change language */}
              <button
                onClick={testChangeLanguage}
                className="mt-4 px-6 py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white rounded-lg text-sm font-semibold transition-all duration-300 shadow-lg shadow-purple-500/25"
              >
                🔄 Change Language (Current: {language.toUpperCase()})
              </button>
            </div>
          </div>
        </div>
