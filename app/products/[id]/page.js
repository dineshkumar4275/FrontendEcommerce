...
//     { name: 'Green', code: '#22c55e', class: 'bg-green-500' },
//     { name: 'Black', code: '#1f2937', class: 'bg-gray-900' },
//     { name: 'White', code: '#ffffff', class: 'bg-white border border-gray-300' },
//     { name: 'Purple', code: '#9333ea', class: 'bg-purple-500' },
//   ];
...
//             {/* Product Images Section with Amazon-Style Zoom */}
//             <div>
...
//                     {hasVideo && (
//                       <span className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-blue-500 to-cyan-500 text-white text-sm font-bold shadow-lg flex items-center gap-1">
//                         <PlayIcon className="w-4 h-4" />
//                         Video
//                       </span>
//                     )}
//                     {hasMultipleImages && (
//                       <span className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white text-sm font-bold shadow-lg flex items-center gap-1">
//                         🔄 360°
//                       </span>
//                     )}
//                   </div>
...
//                   {/* 360° Instruction */}
//                   {hasMultipleImages && isDragging && !showZoom && (
//                     <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/70 text-white text-xs px-3 py-1.5 rounded-full flex items-center gap-2 z-20">
//                       <span className="animate-pulse">🖱️</span>
//                       Dragging to rotate 360°
//                     </div>
//                   )}
...
//                       className="absolute inset-0 z-20 flex items-center justify-center bg-black/40 hover:bg-black/50 transition-all group"
//                     >
...
//                   {selectedImage + 1} / {productImages.length} images • Hover to zoom • Drag to rotate
//                 </div>
//               )}
//             </div>
...
//                   disabled={adding || product.stock === 0 || !selectedSize}
...
export default function ProductDetailPage() {
  ...
  // 🎯 Get sizes from product data (dynamic)
  ...
  // 🎯 Get colors from product data (dynamic)
  const getProductColors = () => {
    if (product?.colors && product.colors.length > 0) {
      return product.colors;
    }
    // Default colors if none in product
    return [
  ...
  useEffect(() => {
    if (product) {
      ...
      if (productColors.length > 0 && !selectedColor) {
        const firstColor = typeof productColors[0] === 'object' ? productColors[0].name : productColors[0];
  ...
  // 🎯 IMPORTANT: Handle color change - changes product image
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div>
              ...
              {/* 🎯 SIZE SELECTOR - DYNAMIC FROM PRODUCT */}
              ...
              {/* 🎯 COLOR SELECTOR - DYNAMIC FROM PRODUCT & CHANGES IMAGE */}
                  ...
                  <div className="flex gap-3 flex-wrap">
                    {productColors.map((color) => {
                      ...
                      const colorCode = typeof color === 'object' ? color.code : 
                        { 'Red': '#ef4444', 'Blue': '#3b82f6', 'Green': '#22c55e', 'Black': '#1f2937', 'White': '#ffffff', 'Purple': '#9333ea', 'Pink': '#ec4899', 'Yellow': '#eab308', 'Orange': '#f97316' }[colorName] || '#cccccc';
