...
//                     key={currentImage}
//                   />
//                   {/* Badges - Only showing discount but no sale */}
...
//               {/* Quantity Selector - Visible but disabled */}
//               <div className="mb-6">
//                 <label className="block text-sm font-semibold text-gray-700 mb-3">Quantity</label>
//                 <div className="flex items-center gap-4">
//                   <div className="flex items-center gap-2 bg-gray-100 rounded-xl p-1">
...
import { getProductById, getCategories } from '../../../src/services/productService';
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `${product.name} - Premium Product | Sombu Store`,
    description: product.description || `Buy ${product.name} online at best price.`,
    keywords: `${product.name}, buy ${product.name}, premium products`,
    alternates: {
      canonical: `https://www.sombu.in/products/${id}`,
    },
    openGraph: {
      title: `${product.name} - Premium Product | Sombu Store`,
      description: product.description || `Buy ${product.name} online at best price.`,
      url: `https://www.sombu.in/products/${id}`,
      siteName: 'Sombu Store',
      images: [
        {
          url: product.image_url || '/favicon.ico',
          width: 1200,
          height: 630,
          alt: product.name,
        },
    ...
    twitter: {
      ...
      title: `${product.name} - Premium Product | Sombu Store`,
      description: product.description || `Buy ${product.name} online at best price.`,
      images: [product.image_url || '/favicon.ico'],
    },
  };
}
...
export default async function ProductDetailPage({ params }) {
  ...
  const breadcrumbs = [
    { name: 'Home', url: 'https://www.sombu.in/' },
    { name: 'Products', url: 'https://www.sombu.in/products' },
    { name: product?.name || 'Product', url: `https://www.sombu.in/products/${id}` },
  ];
