...
export default function TrackOrderPage() {
  ...
  const trackingSteps = [
    { key: 'confirmed', label: t('confirmed') || 'Confirmed', icon: '✅' },
    { key: 'accepted', label: t('assigned') || 'Assigned', icon: '👨‍✈️' },
    { key: 'picked_up', label: t('picked') || 'Picked', icon: '📦' },
    { key: 'in_transit', label: t('shipped') || 'Shipped', icon: '🚚' },
    { key: 'out_for_delivery', label: t('out_for_delivery') || 'Near You', icon: '🏠' },
    { key: 'delivered', label: t('delivered') || 'Delivered', icon: '🎉' }
  ];
    ...
    return (
      <>
        ...
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            <div className="text-6xl mb-4">🔍</div>
            <h2 className="text-2xl font-bold mb-2">{t('order_not_found') || 'Order Not Found'}</h2>
            <p className="text-gray-500 mb-4">{t('could_not_find_order') || `We couldn't find order #${orderId}`}</p>
            <Link href="/track" className="text-purple-600 hover:underline">
              {t('try_another_order') || 'Try another order ID →'}
            </Link>
          </div>
        </div>
        <Footer />
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-purple-50/30 py-8 pt-24">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-6">
              <div className="flex items-center gap-2 text-sm text-gray-500">
                <Link href="/" className="hover:text-purple-600 transition">{t('home') || 'Home'}</Link>
                <span>/</span>
                <Link href="/track" className="hover:text-purple-600 transition">{t('track_order') || 'Track Order'}</Link>
                <span>/</span>
                <span className="text-purple-600 font-medium">#{order.order_number || order.id}</span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
              <div className="flex flex-wrap justify-between items-center gap-4">
                <div>
                  <h1 className="text-2xl font-bold">{t('track_order') || 'Track Order'}</h1>
                  <p className="text-gray-500 mt-1">{t('order') || 'Order'} #{order.order_number || order.id}</p>
                </div>
                <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold ${getStatusColor(order.status)}`}>
                  {getStatusIcon(order.status)}
                  {t(order.status) || order.status?.toUpperCase() || 'PENDING'}
                </span>
              </div>
            </div>
            ...
            {/* Tracking Timeline */}
            <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
              <h2 className="text-lg font-semibold mb-6">{t('order_status') || 'Order Status'}</h2>
              ...
              {order.tracking && order.tracking.length > 0 && (
                <div className="mt-6 pt-6 border-t">
                  <h3 className="font-semibold mb-4">{t('tracking_history') || 'Tracking History'}</h3>
                  <div className="space-y-4 max-h-80 overflow-y-auto">
                    {order.tracking.map((track, idx) => (
                      <div key={idx} className="flex gap-3">
                        <div className="flex-shrink-0">
                        ...
                        <div>
                          <p className="font-medium capitalize text-gray-800">{t(track.status) || track.status?.replace(/_/g, ' ')}</p>
                          <p className="text-sm text-gray-500">{track.location}</p>
                          <p className="text-xs text-gray-400">{formatDate(track.created_at || track.timestamp)}</p>
                        </div>
                      </div>
            ...
            {/* Order Info */}
            <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
              <h2 className="text-lg font-semibold mb-4">{t('order_information') || 'Order Information'}</h2>
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  <p className="text-sm text-gray-500">{t('payment_method') || 'Payment Method'}</p>
                  <p className="font-medium capitalize">{order.payment_method || order.paymentMethod || 'COD'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">{t('payment_status') || 'Payment Status'}</p>
                  <p className="font-medium text-green-600 capitalize">{order.payment_status || order.paymentStatus || 'Paid'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">{t('total_amount') || 'Total Amount'}</p>
                  <p className="font-bold text-purple-600 text-lg">₹{formatAmount(order.total_amount)}</p>
                </div>
              </div>
            </div>
            ...
            {items.length > 0 && (
              <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
                <h2 className="text-lg font-semibold mb-4">{t('items_ordered') || 'Items Ordered'}</h2>
                <div className="space-y-4">
                  {items.map((product, idx) => (
                    <div key={idx} className="flex gap-4 pb-4 border-b border-gray-100 last:border-0">
                      <div className="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                      ...
                      <div className="flex-1">
                        <p className="font-medium">{product.name}</p>
                        <p className="text-sm text-gray-500">{t('quantity') || 'Quantity'}: {product.quantity}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-purple-600">₹{formatAmount(product.price)}</p>
                        <p className="text-xs text-gray-400">{t('total') || 'Total'}: ₹{formatAmount(product.price * product.quantity)}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {address && (
              <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
                <h2 className="text-lg font-semibold mb-4">{t('shipping_address') || 'Shipping Address'}</h2>
                <div className="text-gray-700">
                  {typeof address === 'object' ? (
                    <>
                      <p className="font-medium">{address.fullName}</p>
                      <p>{address.address}</p>
                      <p>{address.city}, {address.state} - {address.pincode}</p>
                      <p>{t('phone') || 'Phone'}: {address.phone}</p>
                    </>
                  ) : (
                    <p>{address}</p>
                  )}
