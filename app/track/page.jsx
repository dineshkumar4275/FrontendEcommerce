...
export default function TrackOrderInputPage() {
  ...
  const handleTrack = async (e) => {
    ...
    if (!orderId.trim()) {
      toast.error(t('enter_order_id') || 'Please enter your order ID');
      return;
    }
    ...
    if (!orderExists) {
      toast.error(t('order_not_found') || 'Order not found. Please check your order ID.');
      setLoading(false);
      return;
    }
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="mb-6">
              <div className="flex items-center gap-2 text-sm text-gray-500">
                ...
                <span>/</span>
                <span className="text-purple-600 font-medium">{t('track_order') || 'Track Order'}</span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
              <div className="bg-gradient-to-r from-purple-600 to-pink-600 px-6 py-8 text-center">
                ...
                <h1 className="text-2xl font-bold text-white">{t('track_your_order') || 'Track Your Order'}</h1>
                <p className="text-white/80 mt-2">{t('track_order_description') || 'Enter your order ID to track your package'}</p>
              </div>
              ...
              <div className="p-8">
                <form onSubmit={handleTrack} className="space-y-6">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                      {t('order_id') || 'Order ID / Order Number'}
                    </label>
                    <div className="relative">
                      <input
                        type="text"
                        value={orderId}
                        onChange={(e) => setOrderId(e.target.value)}
                        placeholder={t('enter_order_id') || 'Enter your order ID'}
                        className="w-full px-4 py-3 pl-12 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                        autoFocus
                      />
                      <MagnifyingGlassIcon className="absolute left-4 top-3.5 h-5 w-5 text-gray-400" />
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      {t('order_id_helper') || 'You can find your order ID in your order confirmation email or in your orders page.'}
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white py-3 rounded-xl font-semibold hover:shadow-lg transition disabled:opacity-50"
                  >
                    {loading ? t('checking') || 'Checking...' : `${t('track_order') || 'Track Order'} →`}
                  </button>
                </form>
                ...
                {recentOrders.length > 0 && (
                  <div className="mt-8 pt-6 border-t border-gray-100">
                    <p className="text-sm font-medium text-gray-700 mb-3">{t('recent_orders') || 'Recent Orders'}</p>
                    <div className="space-y-2">
                      {recentOrders.map((order) => (
                        <button
                          key={order.id}
                        ...
                        >
                          <div>
                            <p className="text-sm font-medium text-gray-800">#{order.id}</p>
                            <p className="text-xs text-gray-500">
                              {order.created_at ? new Date(order.created_at).toLocaleDateString() : t('recent_order') || 'Recent order'}
                            </p>
                          </div>
                          <span className="text-purple-600 text-sm">{t('track') || 'Track'} →</span>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                <div className="mt-6 text-center">
                  <p className="text-sm text-gray-500">
                    {t('no_order_id') || "Don't have an order ID?"} 
                    <Link href="/orders" className="text-purple-600 hover:underline ml-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
              <div className="bg-white rounded-xl p-4 text-center shadow-sm">
                ...
                <h3 className="font-semibold">{t('order_confirmed') || 'Order Confirmed'}</h3>
                <p className="text-xs text-gray-500 mt-1">{t('order_placed') || 'Your order has been placed'}</p>
              </div>
              <div className="bg-white rounded-xl p-4 text-center shadow-sm">
                <div className="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-3">
                  <span className="text-2xl">🚚</span>
                </div>
                <h3 className="font-semibold">{t('shipped') || 'Shipped'}</h3>
                <p className="text-xs text-gray-500 mt-1">{t('on_the_way') || 'Your order is on the way'}</p>
              </div>
              <div className="bg-white rounded-xl p-4 text-center shadow-sm">
                <div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-3">
                  <span className="text-2xl">🏠</span>
                </div>
                <h3 className="font-semibold">{t('delivered') || 'Delivered'}</h3>
                <p className="text-xs text-gray-500 mt-1">{t('delivered_success') || 'Order delivered successfully'}</p>
              </div>
            </div>
            ...
            <div className="mt-6 text-center">
              <p className="text-xs text-gray-400">
                {t('need_help') || 'Need help?'} <Link href="/contact" className="text-purple-600 hover:underline">{t('contact_support') || 'Contact Support'}</Link>
              </p>
            </div>
          </div>
        </div>
