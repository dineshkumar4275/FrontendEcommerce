...
export const CountrySelector = () => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-72 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 z-50 max-h-96 flex flex-col">
            ...
            {filteredCountries.map((c) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-2.5 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors ${
                  country === c.code ? 'bg-purple-50 dark:bg-purple-900/30' : ''
                }`}
              >
                <span className="text-xl">{c.flag}</span>
                <div className="flex-1 text-left">
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{c.name}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">{c.code} • {currencies.find(curr => curr.code === c.currency)?.symbol} {c.currency}</p>
                </div>
                {country === c.code && (
