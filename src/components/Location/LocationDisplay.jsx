...
const LocationDisplay = () => {
  ...
  // ✅ Detect location - Main function (GPS First with better error handling)
  const detectLocation = async () => {
    ...
    setError(null);
    setToastShown(false);
    setGpsAttempted(false);

    try {
      // ✅ Check if GPS is available
      if (navigator.geolocation) {
        console.log('📍 GPS is available. Requesting location...');
        setGpsAttempted(true);
        
        // ✅ Get GPS position with better options
        navigator.geolocation.getCurrentPosition(
          async (position) => {
            const { latitude, longitude, accuracy } = position.coords;
            console.log('📍 GPS Position:', { latitude, longitude, accuracy });
            
            // ✅ Check if GPS accuracy is good (less than 100m)
            if (accuracy > 100) {
              console.log('⚠️ GPS accuracy is low:', accuracy, 'meters');
              toast.warning('Location accuracy is low. Please enable high accuracy GPS.');
            }
    ...
    try {
      ...
      if (response.data.success && response.data.data) {
        if (!locationSet) {
          ...
          if (!toastShown && !locationSet) {
            // ✅ Show warning that IP location might be inaccurate
            toast.warning(`📍 ${displayStreet} detected (IP based - may be inaccurate)`);
            setToastShown(true);
            setLocationSet(true);
          }
        }
  ...
  const getMethodBadge = () => {
    ...
    const badges = {
      'gps': { label: 'GPS', color: 'text-green-400 bg-green-500/20', icon: '📡' },
      'ip': { label: 'IP', color: 'text-yellow-400 bg-yellow-500/20', icon: '🌐' },
      'search': { label: 'Search', color: 'text-blue-400 bg-blue-500/20', icon: '🔍' },
      'browser': { label: 'GPS', color: 'text-green-400 bg-green-500/20', icon: '📡' },
    };
    ...
    return (
      <div className="flex items-center gap-2 px-2 sm:px-3 py-1.5 sm:py-2 bg-purple-500/10 rounded-lg animate-pulse">
        ...
        <span className="text-xs sm:text-sm text-purple-300">
          {gpsAttempted ? 'Getting GPS...' : 'Detecting...'}
        </span>
                      ...
                      <div className="flex items-start gap-2 sm:gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            {locationMethod === 'ip' && (
                              <span className="text-[8px] sm:text-[10px] text-yellow-400/60">
                                ⚠️ May be inaccurate
                              </span>
                            )}
                          </div>
                          {locationMethod === 'ip' && (
                            <p className="text-[8px] sm:text-[10px] text-yellow-400/50 mt-1">
                              Tip: Enable GPS for accurate location
                            </p>
                          )}
                        </div>
                      </div>
