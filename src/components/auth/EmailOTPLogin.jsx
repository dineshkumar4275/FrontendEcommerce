...
//   );
// }
// frontend/app/login/otp/page.js or frontend/components/auth/EmailOTPLogin.jsx
'use client';
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (process.env.NODE_ENV === 'development' && response.data.data?.otp) {
          console.log('📱 Test OTP:', response.data.data.otp);
          toast.success(`📱 Test OTP: ${response.data.data.otp}`, { duration: 10000 });
        }
      }
    } catch (error) {
      toast.error(error.response?.data?.message || 'Failed to send OTP');
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (userRole === 'admin' || user.email === 'admin@example.com') {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.success('OTP resent successfully');
        if (process.env.NODE_ENV === 'development' && response.data.data?.otp) {
          console.log('📱 New Test OTP:', response.data.data.otp);
        }
      }
    } catch (error) {
        ...
        <div className="text-center mb-8">
          ...
          {step === 'otp' && (
            <p className="text-sm text-gray-500 mt-1">
              OTP sent to <strong className="text-[#FF9900]">{email}</strong>
            </p>
          )}
        </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <input 
                ...
                className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-[#FF9900]" 
            ...
            <button 
              ...
              className="w-full bg-gradient-to-r from-[#FF9900] to-[#FF8800] text-white py-3 rounded-lg font-semibold disabled:opacity-50"
            >
              {loading ? 'Sending OTP...' : 'Continue with Email'}
            </button>
            <div className="p-4 bg-orange-50 rounded-lg border border-orange-200">
          ...
          <div className="space-y-6">
            <div>
              ...
              <input 
                ...
                maxLength={6} 
                className="w-full text-center text-2xl py-3 border-2 border-gray-200 rounded-lg tracking-wider focus:outline-none focus:border-[#FF9900]" 
                placeholder="000000" 
                onKeyPress={(e) => e.key === 'Enter' && handleVerifyOTP()} 
              />
              <p className="text-xs text-gray-500 text-center mt-2">
                OTP expires in <span className="font-semibold text-[#FF9900]">{timer}</span> seconds
              </p>
            </div>
            <button 
            ...
            <div className="text-center">
              <button 
                ...
                className="text-sm text-[#FF9900] disabled:text-gray-400 hover:underline"
              >
                {resendDisabled ? `Resend OTP in ${timer}s` : 'Resend OTP'}
              </button>
            </div>
